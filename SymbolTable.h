...
// Searches current subtree (current and underlying scopes) for identifier. returns record if found. returns NULL if not
Record_u* STDeepLookUp(SymbolTable_s ST[static 1], const char* identifier);

// Searches current subtree (current and underlying scopes) for identifier. if found sets current scope to scope containing record and returns record. returns NULL if not
...
// searches current scope for identifier and set it as the new current scope. returns the record for the new scope if successful. returns null if identifier could not be a scope
Record_u* STEnterScope(SymbolTable_s ST[static 1], const char* identifier);

// Exits current scope, sets the current scope to be the parent scope. returns the new current scope. if called in root, current scope is unchanged and NULL is returned
